...
window.onload = function () {
	...
	selectuserid(lStorage.saleman_id);
...
function dealEvents() {
	...
	$('#team_select').on('change', function () {
		getProvince(this).then(function (obj) {
			getCitys(obj)
		}, function (reason) {
...
function initGroups() {
	http.postAjax_clean("/photo-album/teammanger/get_team", {}, function (res) {
		var code = res.code;
		if (code == 0 && res.data) {
			var results = res.data;
			if (results.length > 0) {
				var html = '<option></option>';
				$(results).each(function (key, obj) {
...
function initProAndCity() {
	var proHtml = '<option value="noSelected">-请先选择所属团队-</option>';
	var cityHtml = '<option value="noSelected">-请先选择省份-</option>';
	$('#location_p').html(proHtml);
	$('#location_c').html(cityHtml);
}

// 根据团队获取省份信息
function getProvince(thisElm) {
	var promiseObj = new Promise(function (resolve, reject) {
		var selectedVal = $(thisElm).children('option:selected').val(); // option的value作为areaId
		if (selectedVal != 'noSelected') {
			var post_data = new FormData();
			post_data.append('areaId', selectedVal);
			http.postAjax_clean("/photo-album/areamanager/findByListSaleAreas", post_data, function (res) {
				var code = res.code;
				if (code == 0) {
					var results = res.data[0];
					var html = '<option value="' + results.id + '">' + results.name + '</option>';
...
function getCitys(obj) {
	var post_data = new FormData();
	post_data.append('areaId', obj.areaId);
	post_data.append('proId', obj.proId);
	http.postAjax_clean("/photo-album/areamanager/findByProvidCity", post_data, function (res) {
		var code = res.code;
		if (code == 0) {
			var results = res.data;
			var html = '';
...
function winonload() {
	//销售人员列表接口
	var post_data = new FormData();
	post_data.append('saleTeamId', '');
	post_data.append('name', $('#search_man_name_input').val());
	http.postAjax_clean("/photo-album/sale/saleLoad", post_data, function (data) {
		// console.log(data);
		if (data.code == 0) {
			if (data.data != null) {
				...
				$(data.data.sBeans).each(function (i, ielem) {
					// console.log(ielem);
					customer_manage_html += '<li id="' + ielem.id + '" class="saleman_list_li"><input id="select-' + ielem.id + '" name="select_userid_checkbox" type="checkbox" style="float:left;margin:18px 0 0 30px;"/><button class="select_userid_btn" onclick="selectuserid(\'' + ielem.id + '\')">' + ielem.saleName + '</button></li>';
				});
				$("#customer_list").html(customer_manage_html);
			} else {
				$('#customer_list').html('<li>没有搜到合适的销售人员，请重新输入内容！</li>');
			}
		} else {
			alert(data.msg);
		}
		// http.getAjax_clean("/photo-album/sellmanger/get_sellers", function(data) {
		// console.log(data);
		// var customer_manage_html="";
		// for(var i=0;i<data.length;i++){
		// 	customer_manage_html+='<li id="'+data[i].number+'" class="saleman_list_li"><input id="select-'+data[i].number+'" name="select_userid_checkbox" type="checkbox" style="float:left;margin:18px 0 0 30px;"/><button class="select_userid_btn" onclick="selectuserid(\''+data[i].number+'\')">'+data[i].name+'</button></li>';
		// }
		// $("#customer_list").append(customer_manage_html);
	});
	//跟进客户列表
	// customer_list_js();
	...
	http.getAjax_clean("/photo-album/job/getListFull", function (data) {
		...
		$('#jobId').html('<option value=" "> </option>');
		$(data.data.jobList).each(function (k, kelem) {
			$('#jobId').append('<option value="' + kelem.id + '">' + kelem.jobname + '</option>');
		});
	});
	...
	$(NoAuthMenu_data).each(function (o, oelem) {
		if (oelem.type == 1) {
			...
			switch (oelem.menuname) {
				...
				case '添加销售人员':
					// console.log(oelem);
					$('#add_saleman_btn').hide();
					$('#saleman_submit_div').hide();
					break;
				// 删除销售人员
				case '删除销售人员':
					// console.log(oelem);
				...
				case '删除跟进客户':
					// console.log(oelem);
					$('#delete_seller_btn').hide();
					break;
				// 添加到团队
				case '添加到团队':
					// console.log(oelem);
					$('#customer_addtoteam_btn').hide();
					break;
			}
		}
	});
...
//添加跟进客户按钮JS
function add_seller_btn() {
	var post_data = new FormData();
	post_data.append('saleId', saleman_id);
	http.postAjax_clean("/photo-album/customer/getCustom", post_data, function (data) {
		if (data.code == 0) {
			var customer_list_html = '';
			$(data.data).each(function (i, elem) {
				// console.log(elem.id);
				// console.log($('.NoSaved_li'));
				//判断是否已经添加过客户，
				var elemid_tf = true;
				$('.NoSaved_li').each(function (j, jelem) {
					// console.log(($(jelem)[0].firstChild.id).split('ded_')[1]);
					if (elem.id == ($(jelem)[0].firstChild.id).split('ded_')[1]) {
						elemid_tf = false;
					}
				});
				//判断是否已经添加过客户，已经添加则过滤掉
				if (elemid_tf) {
					if (elem.twoaccount) {
						customer_list_html += '\
							<li class="NoSale_li"><input id="'+ elem.id + '" class="select-checkbox" name="' + elem.oneaccount + '-' + elem.twoaccount + '" type="checkbox" style="float:left;margin:18px 0 0 30px;"/> ' + elem.oneaccount + '-' + elem.twoaccount + ' </li>\
						';
					} else {
						customer_list_html += '\
							<li class="NoSale_li"><input id="'+ elem.id + '" class="select-checkbox" name="' + elem.oneaccount + '" type="checkbox" style="float:left;margin:18px 0 0 30px;"/> ' + elem.oneaccount + ' </li>\
						';
					}
				}
			});
			$("#customer_follow_list").html(customer_list_html);
		} else {
			alert(data.msg);
		}
	});
...
//所属团队列表
function teamlist_display() {
	//	// var post_data = new FormData();
	//	// post_data.append('teamname', '');
	//	// http.postAjax_clean("/photo-album/teammanger/get_team", post_data,function(data) {
	//	http.postAjax_clean("/photo-album/teammanger/get_team", '',function(data) {
	//		// console.log(data);
	//		if(data.code == 0){
	//			var team_select_html = '<option selected=" "> </option>';
	//			$(data.data).each(function(i,ielem){
	//				// console.log(ielem);
	//				team_select_html +='<option value="'+ielem.id+'">'+ielem.name+'</option>';
	//			});
	//			$('#team_select').html(team_select_html);
	//		}else{
	//			alert(data.msg);
	//		}
	//	});
...
//跟进客户“提交”按钮
function add_customer_follow_btn() {
	var saleId = saleman_id;
	var custIdsList = [];
	var checkedList = $('#customer_follow_list').find('input:checked');
	if (saleId && saleId != '') {
		if (checkedList.length > 0) {
			$.each(checkedList, function (key, item) {
				custIdsList.push($(item).attr('id'));
			})

			var post_data = new FormData();
			post_data.append('saleId', saleId);
			post_data.append('customerId', custIdsList);
			http.postAjax_clean('/photo-album/sale/addSaleFollowCustomer', post_data, function (res) {
				if (res.code == '0') {
					location.reload();
				} else {
					alert(res.msg);
	...
	//	add_customer_follow_list = [];
	//	//nnnn
	//	$('.NoSaved_li').each(function(k,kelem){
	//		var customer_list_temp = {};
	//		customer_list_temp.cusId = ($(kelem)[0].firstChild.id).split('ded_')[1];
	//		customer_list_temp.cusName = $(kelem)[0].innerText;
	//		add_customer_follow_list.push(customer_list_temp);
	//	});
	//
	//	$($('#customer_follow_list').find('input:checked')).each(function(i,elem){
	//		// console.log(elem);
	//		// console.log($(elem)[0].id);
	//		var customer_list_temp = {};
	//		customer_list_temp.cusId = $(elem)[0].id;
	//		customer_list_temp.cusName = $(elem)[0].name;
	//		add_customer_follow_list.push(customer_list_temp);
	//	});
	//	//跟进客户数组add_customer_follow_list
	//	// console.log(add_customer_follow_list);
	//
	//	//如果客户保存有重复返回，则不执行以下js
	//	// console.log(saleman_save_btn());
	//	// if(saleman_save_btn(2) != false){
	//		//跟进客户数组add_customer_follow_list去重复
	//		removeDuplicates(add_customer_follow_list);
	//		// console.log(add_customer_follow_list);
	//
	//		//提交跟进客户数组到右方
	//		added_customer_list_js(add_customer_follow_list);
	//		//关闭弹出层
	//		$('#myModal').modal('hide');
	//	// }else{
	//	// 	//关闭弹出层
	...
	//			//arr[i]跟后边的比较，比较相同就将数组重复的去掉
	//			// console.log(arr[i]);
	//			for(var j=(i+1);j<arr.length;j++){
	//				if(arr[i].cusName == arr[j].cusName && arr[i].cusId == arr[j].cusId ){
	//					arr.splice(j, 1);
	//					break;
	//				}
	//			}
...
function add_saleman_btn(elem) {
	...
	$("#number").show();
	$("#saleman_save_div").hide();
	$("#saleman_submit_div").show();
	$('#customer_followed_list').html('暂无跟进客户，请按"添加跟进客户"按钮添加客户！');
	$(".saleman_list_li").each(function (i, subelem) {
		$(subelem).css('background-color', '#fff');
		$(subelem).css('color', '#000');
	});

}

//"创建新员工"保存按钮
function saleman_submit_btn() {
	...
	http.postAjax_clean("/photo-album/sale/saleLoad", post_data, function (data) {
		if (data.code == 0) {
			//判断姓名和工号字段不能为空，密码为6位数字。
			// console.log($("#name").val());
			// console.log($("#number").val());
			// console.log($("#nikename").val());
			if (($("#name").val() != null) && ($("#nikename").val() != '') && ($("#number").val() != null) && ($("#password").val().length == 6)) {
				//添加新员工
				var post_data = new FormData();
				post_data.append('saleTeamId', $("#team_select").val());
				post_data.append('jobId', $("#jobId").val());
				post_data.append('name', $("#name").val());
				post_data.append('nikename', $("#nikename").val());
				post_data.append('number', $("#number").val());
				post_data.append('sex', $("#sex").val());
				post_data.append('phone', $("#phone").val());
				post_data.append('province', $("#location_p").val());
				post_data.append('city', $("#location_c").val());
				post_data.append('password', $("#password").val());
				var customer_select_list = [];
				$('.fed_class').each(function (k, kelem) {
					// console.log(kelem.value);
					customer_select_list.push((kelem.id).split('ded_')[1]);
				});
				// console.log(customer_select_list);
				post_data.append('customerid', customer_select_list);
				http.postAjax_clean("/photo-album/sale/add_seller", post_data, function (data) {
					if (data.code == 0) {
						// console.log(data.data);
						var lStorage = window.localStorage;
						lStorage.saleman_id = data.data.id;
						alert("创建销售人员成功");
						location.reload();
					} else {
						alert(data.msg);
...
//“删除员工”按钮
function del_saleman_btn() {
	var will_delete_del_saleman = $('#customer_list').find('input:checked');
	// console.log(will_delete_del_saleman);
	$(will_delete_del_saleman).each(function (i, elem) {
		var post_data = new FormData();
		post_data.append('id', $(elem)[0].id.substring(7));
		http.postAjax_clean("/photo-album/sale/delete_seller", post_data, function (data) {
			// console.log(data);
		});
	});
	add_saleman_btn(2);
...
//"保存"按钮
function saleman_save_btn(elem) {
	if ($("#nikename").val().length <= 8) {
		if (($("#name").val() != null) && ($("#nikename").val() != '') && ($("#number").val() != null) && ($("#password").val().length == 6)) {
			//更新员工资料
			var customer_json = [];
			$('.fed_class').each(function (k, kelem) {
				// console.log(kelem);
				customer_json.push((kelem.id).split('ded_')[1]);
			});
			// console.log(customer_json);
			// console.log(add_customer_follow_list);
			var post_data = new FormData();
			post_data.append('id', saleman_id);
			post_data.append('saleTeamId', $("#team_select option:checked").val());
			post_data.append('jobId', $("#jobId").val());
			post_data.append('name', $("#name").val());
			post_data.append('nikename', $("#nikename").val());
			post_data.append('number', $("#number").val());
			...
			post_data.append('city', $("#location_c").val());
			post_data.append('password', $("#password").val());
			post_data.append('customerid', customer_json);
			// post_data.append('customerid', 'aaa');
			http.postAjax_clean("/photo-album/sale/add_seller", post_data,
				function (data) {
					if (data.code == 0) {
						// console.log(data.data);
						var lStorage = window.localStorage;
...
//已添加“跟进客户列表”在右方显示
function added_customer_list_js(elem) {
	// console.log(elem);
	if (elem.length != 0) {
		$('#customer_displaydiv').hide();
		var customer_followed_list_html = '';
		$(elem).each(function (j, fedelem) {
			// console.log(elem);
			//页面跟进客户数组
			customer_followed_list_html += '\
				<li class="NoSaved_li"><input id="ded_'+ fedelem.cusId + '" data-myid="' + fedelem.cusId + '" class="fed_class" name="Fruit" type="checkbox" style="float:left;margin:18px 0 0 30px;"/> ' + fedelem.cusName + '</li>\
			';
		});
		$('#customer_followed_list').html(customer_followed_list_html);
	} else {
		$('#customer_followed_list').html('<div id="customer_displaydiv">暂无跟进客户，请按"添加跟进客户"按钮添加客户！</div>');
	}
}

//已添加“跟进客户列表”在右方显示
function seller_search_input() {
	var lStorage = window.localStorage;
	var post_data = new FormData();
	post_data.append('saleId', lStorage.saleman_id);
	post_data.append('name', $('#seller_search_input').val());
	http.postAjax_clean("/photo-album/sale/likeSale", post_data, function (data) {
		// console.log(data);
		if (data.code == 0) {
			// console.log(data.data.custors);
			if (data.data.custors != null) {
				var customer_followed_list_html = '';
				$(data.data.custors).each(function (j, fedelem) {
					//页面跟进客户数组
					customer_followed_list_html += '\
						<li class="Saved_li"><input id="ded_'+ fedelem.cusId + '" class="fed_class" name="Fruit" type="checkbox" style="float:left;margin:18px 0 0 30px;"/> ' + fedelem.cusName + '</li>\
					';
				});
				$('#customer_followed_list').html(customer_followed_list_html);
			} else {
				$('#customer_followed_list').html('<li id="customer_displaydiv">暂无跟进客户，请按"添加跟进客户"按钮添加客户！</li>');
			}
		} else {
			alert(data.msg);
		}
...
//员工选择列表
function selectuserid(elem) {
	// console.log(elem);
	saleman_id = elem;
	// console.log(saleman_id);
	// console.log("saleman_id:"+saleman_id);
	var lStorage = window.localStorage;
	lStorage.saleman_id = elem;
	// console.log(lStorage);

	var post_data = new FormData();
	post_data.append('id', elem);
	http.postAjax_clean("/photo-album/sale/get_seller_detail", post_data, function (data) {
		if (data.code == 0) {
			if (data.data != null) {
				// console.log(data.data);
				$('.select-checkbox').each(function (k, subelem) {
					$(subelem).prop("checked", false);
				});
				$(data.data.tracecustomerid).each(function (j, checkelem) {
					$('#' + checkelem).prop("checked", true);
				});
				if (data.data.tracecustomerid) {
					add_customer_follow_list = data.data.tracecustomerid;
				} else {
					add_customer_follow_list = [];
				}
				// console.log(add_customer_follow_list);
				new PCAS('location_p', 'location_c', data.data.province, data.data.city);
				$("#name").val(data.data.name);
				$("#nikename").val(data.data.nikename);
				...
				$("#team_select").val(data.data.saleTeamId);
				...
				setTimeout(function () {
					http.postAjax_clean("/photo-album/sale/likeSale", post_data, function (data) {
						if (data.code == 0) {
							// console.log(data.data.custors);
							if (data.data.custors != null) {
								added_customer_list_js(data.data.custors);
							} else {
								$('#customer_followed_list').html('<div id="customer_displaydiv">暂无跟进客户，请按"添加跟进客户"按钮添加客户！</div>');
							}
						} else {
				...
				$("#city").val('');
				var temp_list = [];
				added_customer_list_js(temp_list);
			}
		} else {
			alert(data.msg);
		}
	...
	setTimeout(function () {
		$(".saleman_list_li").each(function (i, subelem) {
			if ($(subelem)[0].id == elem) {
				$(subelem).css('background-color', '#2a6aa3');
				$(subelem).css('color', '#fff');
			} else {
				$(subelem).css('background-color', '#fff');
				$(subelem).css('color', '#000');
			}
...
//"删除跟进客户列表"按钮
function delete_seller_btn() {

	var saleId = saleman_id;
	var custIdsList = [];
	var checkedList = $('#customer_followed_list').find('input:checked');
	$.each(checkedList, function (key, item) {
		custIdsList.push($(item).data('myid'));
	})
	if (custIdsList && custIdsList.length > 0) {
		var post_data = new FormData();
		post_data.append('saleId', saleId);
		post_data.append('customerId', custIdsList);
		http.postAjax_clean('/photo-album/sale/deleteSaleFollowCustomer', post_data, function (res) {
			if (res.code == '0') {
				location.reload();
			} else {
				alert(res.msg);
	...
	//	if(saleman_id){
	//		if(($('#customer_followed_list').find('input:checked')).length !=0){
	//			if(confirm("确认删除跟进客户？")){
	//				var customer_del_list = [];
	//				$('#customer_followed_list').find('input:checked').each(function(i,elem){
	//						add_customer_follow_list.splice($.inArray($(elem)[0].id,add_customer_follow_list),1);
	//						// console.log(elem.id);
	//						customer_del_list.push((elem.id).split("ded_")[1]);
	//						$($(elem)[0].parentElement).remove();
	//				});
	//				// console.log(customer_del_list);
	//				// console.log(saleman_id);
	//
	//				// var post_data = new FormData();
	//				// post_data.append('id', saleman_id);
	//				// post_data.append('customerId', customer_del_list);
	//				// http.postAjax_clean("/photo-album/sale/delete_customer_from_seller", post_data,function(data) {
	//				// 	if(data.code == 0){
	//				// 		alert("删除跟进客户成功！");
	//				// 		// location.reload();
	//				// 	}else{
