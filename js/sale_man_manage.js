...
//跟进客户列表JS
function customer_list_js(){
	http.getAjax_clean("/photo-album/customer/get_user", function(data) {
		var customer_list_html = '';
		$(data).each(function(i,elem){
			customer_list_html +='\
				<li><input id="'+elem.userid+'" class="select-checkbox" name="Fruit" type="checkbox" style="float:left;margin:18px 0 0 30px;"/>'+elem.userid+'</li>\
			';
		});
...
function add_customer_follow_btn(){
	...
	$($('#customer_follow_list').find('input:checked')).each(function(i,elem){
		...
		add_customer_follow_list.push($(elem)[0].id);
	});
	...
	http.getAjax_clean("/photo-album/sellmanger/get_sellers", function(data) {
		...
		if(sellertf){
			...
			if(($("#number").val()).length != 0){
				...
				post_data.append('position', $("#position").val());
				post_data.append('region', $("#region").val());
				post_data.append('province', $("#location_p").val());
				post_data.append('city', $("#location_c").val());
				post_data.append('tracecustomerid', add_customer_follow_list);
				http.postAjax_clean("/photo-album/sellmanger/add_seller", post_data,
				function(data) {
					if(data.state ==true){
						location.reload();
					}
...
function del_saleman_btn(){
	...
	$(will_delete_del_saleman).each(function(i,elem){
		var post_data = new FormData();
		post_data.append('number',$(elem)[0].id.substring(7));
		http.postAjax_clean("/photo-album/sellmanger/delete_seller", post_data,function(data) {
			// console.log(data);
		});
	});
	add_saleman_btn();
...
//"保存"按钮
function saleman_save_btn(){
		//更新员工资料
		console.log(add_customer_follow_list);
		var post_data = new FormData();
		post_data.append('name', $("#name").val());
		post_data.append('number', $("#number_text").html());
		post_data.append('sex', $("#sex").val());
		post_data.append('phone', $("#phone").val());
		post_data.append('position', $("#position").val());
		post_data.append('region', $("#region").val());
		...
		post_data.append('city', $("#location_c").val());
		post_data.append('tracecustomerid', add_customer_follow_list);
		http.postAjax_clean("/photo-album/sellmanger/update_seller", post_data,
		function(data) {
			if(data.state ==true){
				selectuserid(saleman_id);
				// add_saleman_btn();
				location.reload();
			}
		});
	...
	if(elem.length != 0){
		...
		$(elem).each(function(j,fedelem){
			...
			customer_followed_list_html +='\
				<li><input id="delete-'+fedelem+'" name="Fruit" type="checkbox" value="客户A" style="float:left;margin:18px 0 0 30px;"/>'+fedelem+'</li>\
			';
		});
...
function selectuserid(elem){
	...
	http.postAjax_clean("/photo-album/sellmanger/get_seller_detail", post_data,function(data) {
		$('.select-checkbox').each(function(k,subelem){
			$(subelem).prop("checked",false);
		});
		$(data.tracecustomerid).each(function(j,checkelem){
			$('#'+checkelem).prop("checked",true);
		});
		if(data.tracecustomerid){
			add_customer_follow_list = data.tracecustomerid;
		}else{
			add_customer_follow_list = [];
		}
		// console.log(add_customer_follow_list);
		new PCAS('location_p', 'location_c', data.province, data.city);
		$("#name").val(data.name);
		$("#number_text").html(data.number);
		$("#sex").val(data.sex);
