...
function getProvince() {
	http.getAjax_clean("/photo-album/areamanager/findByAllProviceAndCity", function (res) {
		var code = res.code;
		if (code == 0) {
			allProAndCity = res.data;
			var html = '';
...
// 根据省份select的索引获取所有城市信息【前端做的城市搜索】
function getCitysByProIndex() {
	var index = $('#location_p').find('option:selected').index();
	var citys = allProAndCity[index].cities;
	var html = '';
	$.each(citys, function (key, item) {
...
function winonload() {
	http.getAjax_clean("/photo-album/customer/get_user", function (data) {
		get_user_data_onload(data);
	});
	salemanload();
	//权限管理适配开始
	setTimeout(function () {
			...
			if (oelem.type == 1) {
				...
				switch (oelem.menuname) {
					...
					case '添加客户':
						...
						$('#customer_submit_div').hide();
						break;
					//修改客户
					case '修改客户':
						$('#customer_save_div').hide();
						break;
					//删除客户
					case '删除客户':
						$('#del_customer_btn').hide();
						break;
					//启用/禁用客户
					case '启用/禁用客户':
						$('#forbidden').hide();
					...
					//设置客户密码
					case '设置客户密码':
						$('#passwordsetting').attr("disabled", "false");
						break;
					//设置跟进业务员
					case '设置跟进业务员':
						$('#customerservice').attr("disabled", "false");
						break;
				}
			}
		});
...
//get_user接口data数据读取
function get_user_data_onload(elem) {
	// console.log(elem);
	var customer_manage_html = "";
	for (var i = 0; i < elem.length; i++) {
		// console.log(elem[i]);
		// console.log(elem[i].twoaccount);
		if (elem[i].twoaccount) {
			// console.log(elem[i]);
			if (elem[i].isstop == 0) {
				// if((elem[i].customerservice).length == 0){
				if ((elem[i].customerservice).length == 0 || (elem[i].customerservice)[0] == 'null') {
					customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="color:red;width:100%;" onclick="selectuserid(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '-' + elem[i].twoaccount + '</button></td></tr>';
				} else {
					customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="width:100%;" onclick="selectuserid(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '-' + elem[i].twoaccount + '</button></td></tr>';
				}
			} else {
				customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="color:#ccc;width:100%;" onclick="selectuserid_hidden(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '-' + elem[i].twoaccount + '(已禁用)</button></td></tr>';
			}
		} else {
			// console.log(elem[i]);
			if (elem[i].isstop == 0) {
				// if((elem[i].customerservice).length == 0){
				if ((elem[i].customerservice).length == 0 || (elem[i].customerservice)[0] == 'null') {
					customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="color:red;width:100%;" onclick="selectuserid(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '</button></td></tr>';
				} else {
					customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="width:100%;" onclick="selectuserid(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '</button></td></tr>';
				}
			} else {
				customer_manage_html += '<tr id="id-' + elem[i].userid + '"><td width="40px"><input id="' + elem[i].userid + '" value="' + elem[i].id + '" name="select_userid_checkbox" type="checkbox"/></td><td><button id="select_userid_btn_' + i + '" class="select_userid_btn" style="color:#ccc;width:100%;" onclick="selectuserid_hidden(\'' + elem[i].userid + '\')">' + elem[i].oneaccount + '(已禁用)</button></td></tr>';
			}

		}
		// console.log((data[i].customerservice).length);
	}
	$("#customer_list").html(customer_manage_html);
}

//销售人员数据读取
function salemanload() {
	...
	http.postAjax_clean("/photo-album/sale/saleLoad", post_data, function (data) {
		...
		if (data.code == 0) {
			var sellerlist_text = "<option> </option>";
			$(data.data.sBeans).each(function (s, selem) {
				// console.log(selem);
				sellerlist_text += '<option value="' + selem.id + '">' + selem.saleName + '</option>';
			});
			$(".salemanclass").html(sellerlist_text);
		}
	});
}
...
function customer_submit() {
	//密码输入框判断不能少于6位
	var password_inputtext = $("#passwordsetting").val().replace(/\s/g, "");
	// console.log($("#customerservice").val().length);
	if ($('#oneaccount').val() != '' && $("#customerservice").val() != '' && $('#nikename').val() != '' && $('#userid').val() != '' && $('#location_p').val() != ' ' && $('#location_c').val() != ' ' && $('#contacts').val() != '' && $('#phonenumber').val() != '') {
		if (password_inputtext.length == 6) {
			//判断是否为有效的电话号码格式
			if (checkPhone($('#phonenumber').val())) {
				if (($("#userid").val()).length != 0) {
					//判断业务负责人、业务协助人1、业务协助人2都必须为不同的人员
					if (($('#BusinessManTwo').val() == '' && $('#BusinessManOne').val() == '') || (($('#customerservice').val() != $('#BusinessManOne').val()) && ($('#customerservice').val() != $('#BusinessManTwo').val()) && ($('#BusinessManOne').val() != $('#BusinessManTwo').val()))) {
						var post_data = new FormData();
						post_data.append('oneaccount', $("#oneaccount").val());
						post_data.append('twoaccount', $("#twoaccount").val());
						post_data.append('nikename', $("#nikename").val());
						post_data.append('userid', $("#userid").val());
						post_data.append('region', $("#region").val());
						post_data.append('province', $("#location_p").val());
						post_data.append('city', $("#location_c").val());
						post_data.append('contacts', $("#contacts").val());
						post_data.append('phonenumber', $("#phonenumber").val());
						post_data.append('customerservice', $("#customerservice").val());
						post_data.append('busiassis1', $("#BusinessManOne").val());
						post_data.append('busiassis2', $("#BusinessManTwo").val());
						post_data.append('machineprivilege', $("#machineprivilege").val());
						post_data.append('credit', $("#credit").val());
						post_data.append('freeperiod', $("#freeperiod").val());
						post_data.append('shippingrebate', $("#shippingrebate").val());
						// post_data.append('regionalmanager', $("#regionalmanager").val());
						post_data.append('monthlyrate', $("#monthlyrate").val());
						post_data.append('password', $("#passwordsetting").val());
						post_data.append('isstop', 0);
						http.postAjax_clean("/photo-album/customer/add", post_data,
							function (data) {
								// console.log(data);
								if (data.code == 0) {
									alert('添加客户成功!');
									http.getAjax_clean("/photo-album/customer/get_user", function (data) {
										get_user_data_onload(data);
										var lStorage = window.localStorage;
										lStorage.selected_userid = $("#userid").val();
										// console.log(lStorage.selected_userid);
										selectuserid(lStorage.selected_userid);
									});
								} else {
									alert(data.msg);
									// location.reload();
						...
						alert("业务负责人、业务协助人1、业务协助人2都必须为不同的人员！");
					}
				} else {
					alert("客户编码不能为空，请重新输入！");
					return;
				}
			} else {
				alert('客户联系电话不是有效的电话或手机号码格式！');
			}
		} else {
			alert("密码设置必须为6位！");
		}
	} else {
		alert('打*的必须项不能为空，请填写完整！');
	}
...
function customer_save() {
	if ($('#oneaccount').val() != '' && $("#customerservice").val() && $("#customerservice").val() != '' && $('#nikename').val() != '' && $('#userid').val() != '' && $('#location_p').val() != ' ' && $('#location_c').val() != ' ' && $('#contacts').val() != '' && $('#phonenumber').val() != '') {
		var password_inputtext = $("#passwordsetting").val().replace(/\s/g, "");
		if (password_inputtext.length == 6) {
			//判断是否为有效的电话号码格式
			if (checkPhone($('#phonenumber').val())) {
				...
				if (($('#BusinessManTwo').val() == '' && $('#BusinessManOne').val() == '') || (($('#customerservice').val() != $('#BusinessManOne').val()) && ($('#customerservice').val() != $('#BusinessManTwo').val()) && ($('#BusinessManOne').val() != $('#BusinessManTwo').val()))) {
					http.getAjax_clean("/photo-album/customer/get_user", function (data) {
						...
						$(data).each(function (i, ielem) {
							// console.log(ielem);
							if (ielem.userid == selected_userid) {
								// console.log(ielem);
								isstop_tf = ielem.isstop;
								user_system_id = ielem.id;
							}
						});
						// console.log("isstop_tf的值");
						// console.log(isstop_tf);
						var post_data = new FormData();
						post_data.append('id', user_system_id);
						post_data.append('oneaccount', $("#oneaccount").val());
						post_data.append('twoaccount', $("#twoaccount").val());
						post_data.append('nikename', $("#nikename").val());
						post_data.append('userid', $("#userid").val());
						...
						post_data.append('monthlyrate', $("#monthlyrate").val());
						post_data.append('password', $("#passwordsetting").val());
						post_data.append('isstop', isstop_tf);
						// http.postAjax_clean("/photo-album/customer/set_user", post_data,
						http.postAjax_clean("/photo-album/customer/update", post_data,
							function (data) {
								// console.log(data);
								if (data.code == 0) {
									alert('编辑客户资料成功!');
									http.getAjax_clean("/photo-album/customer/get_user", function (data) {
										get_user_data_onload(data);
										var lStorage = window.localStorage;
										lStorage.selected_userid = selected_userid;
										// console.log(lStorage.selected_userid);
										selectuserid(lStorage.selected_userid);
									});
								} else {
...
//“启用客户”按钮事件
function enable_clients() {
	// console.log($('input[name=select_userid_checkbox]').checked.length);
	var enable_clients_list = [];
	$('input[name=select_userid_checkbox]').each(function (i, ielem) {
		if (ielem.checked) {
			enable_clients_list.push(ielem.value);
		}
	});
	// console.log(enable_clients_list);
	if (enable_clients_list.length != 0) {
		var post_data = new FormData();
		post_data.append('customerids', enable_clients_list);
		post_data.append('isstop', "0");
		http.postAjax_clean("/photo-album/customer/updateCustomerIsstop", post_data, function (data) {
			if (data.code == 0) {
				location.reload();
			} else {
				alert(data.msg);
...
function selectuserid(elem) {
	$("#forbidden").text("禁用客户");
	selected_userid = elem;
	// new PCAS('location_p', 'location_c', '广东省', '深圳市');
	var li_div_id = "#id-" + elem;
	// console.log(li_div_id);
	var customer_li_list = $("#customer_list").find("tr");
	// console.log(customer_li_list);
	for (var j = 0; j < customer_li_list.length; j++) {
		var li_temp_id = "#" + $(customer_li_list[j]).context.id;
		if (li_temp_id == li_div_id) {
			$(li_temp_id).css("background-color", "#428bca");
			$(li_temp_id).css("color", "#fff");
		} else {
			$(li_temp_id).css("background-color", "#fff");
			$(li_temp_id).css("color", "#000");
		}
	}

	// $(li_div_id).css("background-color","#2b6aa2");
	$("#customer_save_div").show();
	$("#customer_submit_div").hide();
	//用户编码不可被修改
	// $("#userid").attr("readonly","readonly");
	http.getAjax_clean("/photo-album/customer/get_user", function (data) {
		for (var i = 0; i < data.length; i++) {
			if (elem == data[i].userid) {
				//$('#location_p').
				//new PCAS('location_p', 'location_c', data[i].province, data[i].city);
				$("#oneaccount").val(data[i].oneaccount);
				$("#twoaccount").val(data[i].twoaccount);
				$("#nikename").val(data[i].nikename);
				$("#userid").val(data[i].userid);
				$("#region").val(data[i].region);
				// $("#location_p").val(data[i].province);
				// $("#location_c").val(data[i].city);
				$("#location_p option[value='" + data[i].province + "']").prop("selected", true);
				getCitysByProIndex();
				$("#location_c option[value='" + data[i].city + "']").prop("selected", true);
				$("#contacts").val(data[i].contacts);
				$("#phonenumber").val(data[i].phonenumber);
				$("#customerservice").val(data[i].customerservice);
				$("#BusinessManOne").val(data[i].busiassis1);
				$("#BusinessManTwo").val(data[i].busiassis2);
				$("#credit").val(data[i].credit);
				$("#freeperiod").val(data[i].freeperiod);
				$("#machineprivilege").val(data[i].machineprivilege);
				$("#shippingrebate").val(data[i].shippingrebate);
				// $("#regionalmanager").val(data[i].regionalmanager);
				$("#monthlyrate").val(data[i].monthlyrate);
				$("#passwordsetting").val(data[i].password);
				phonenumber_check();
				// $("#oneaccount").val(data[i].oneaccount);
			}
		}
...
//左方点击已禁用客户，右方数据显示
function selectuserid_hidden(elem) {
	$("#forbidden").text("启用客户");
	selected_userid = elem;
	// new PCAS('location_p', 'location_c', '广东省', '深圳市');
	var li_div_id = "#id-" + elem;
	var customer_li_list = $("#customer_list").find("li");
	for (var j = 0; j < customer_li_list.length; j++) {
		var li_temp_id = "#" + $(customer_li_list[j]).context.id;
		if (li_temp_id == li_div_id) {
			$(li_temp_id).css("background-color", "#428bca");
...
//"添加新客户"按钮事件
function add_customer_btn() {
	$($("#customer_list")[0].childNodes).each(function (i, ielem) {
		$(ielem).css("background-color", "#fff");
		$(ielem).css("color", "#000");
	});
	$("#oneaccount").val('');
	$("#twoaccount").val('');
	...
	$("#passwordsetting").val('');
	$("#customer_save_div").hide();
	$("#customer_submit_div").show();
	$('#check_mobile_span').html('不是有效的电话号码格式！');
	// $("#userid").attr("readonly",false);
	// new PCAS('location_p', 'location_c', ' ', ' ');
}

//"删除客户"按钮事件
function del_customer_btn() {
	if (confirm("确认删除用户？")) {
		var del_list = [];
		$('input:checkbox').each(function (i, elem) {
			if (elem.checked) {
				del_list.push(elem.id);
			}
		});
		// console.log(del_list.toString());
		if (del_list.length != 0) {
			var post_data = new FormData();
			post_data.append('useridlist', JSON.stringify(del_list));
			http.postAjax_clean("/photo-album/customer/del_user", post_data, function (data) {
				// console.log(data);
				if (data.state == true) {
					alert("删除用户成功！");
					location.reload();
...
//搜索客户名称事件
function search_cutomer(thiselem) {
	// console.log($(thiselem)[0].value);
	var post_data = new FormData();
	post_data.append('name', $("#search_cutomer_input")[0].value);
	http.postAjax_clean("/photo-album/customer/get_user", post_data, function (data) {
		// console.log(data);
		get_user_data_onload(data);
	});
}

//检查联系电话的输入是否为电话号码和手机号码
function phonenumber_check() {
	// console.log($('#phonenumber').val());
	checkPhone($('#phonenumber').val());//调用
	// console.log(checkPhone($('#phonenumber').val()));
}

//检查联系电话的输入是否为电话号码和手机号码
function checkPhone(str) {
	//  checkPhone('13800138000'); //调用正确
	// checkPhone('139888888889');//错误示例
	//座机格式可为01088888888,010-88888888,0955-7777777
	var re = /^0\d{2,3}-?\d{7,8}$/;
	if (re.test(str)) {
		$('#check_mobile_span').html('有效的电话号码格式！');
		return true;
	} else {
		var res = /^1\d{10}$/
		if (res.test(str)) {
