		...
		$(NoAuthMenu_data).each(function (o, oelem) {
			if (oelem.type == 1) {
				...
				switch (oelem.btnmark) {
					...
					//修改快捷推广名称
					case 'setup_input':
						$('.setup_input').each(function (i, ielem) {
							// console.log(ielem.value);
							var setup_text = ielem.value;
							$(ielem).replaceWith('<div>' + setup_text + '</div>');
						});
						// $('#uploadimg').hide();
						break;
					//修改快捷推广数量
					...
					//修改快捷推广类别
					case 'promo_set_btn':
						$('button[name=promo_set_btn]').each(function (i, ielem) {
							$(ielem).hide();
						});
						break;
					//设置快捷推广图标
					case 'promo_imgset_btn':
						$('.myImage_button').each(function (i, ielem) {
							var myImage_html = ($(ielem)[0].outerHTML).split(' data-toggle="modal"')[0] + ($(ielem)[0].outerHTML).split(' data-toggle="modal"')[1];
							$(ielem).replaceWith(myImage_html);
						});
						break;
				}
			}
...
function promotion_highspeed_imglist(num) {
	...
	for (var i = 0; i < num; i++) {
		...
		promotion_imglist_html += '            <div id="new_Product">'
			...
			+ '<button data-toggle="modal" data-target="#myImage' + i + '" class="promo_imgset_btn">'
			+ '<img src="/photo-album/index/image/' + i + '" width="200px" height="200px"/>'
			+ '</button>'
			+ '<br>'
			+ '<div align="center"></div>'
			+ '<button class="btn btn-primary promo_set_btn"  data-toggle="modal" data-target="#myModal' + i + '">设置推广类别</button>'
			+ '</div>'
			+ '</div>';
		//设置推广类别
		openwindow_div_html += '        <div class="modal fade" id="myModal' + i + '" tabindex="-1" role="dialog" aria-labelledby="myModalLabe" aria-hidden="true">'
			...
			+ '<h4 class="modal-title" id="myModalLable">设置推广类别--' + (i + 1) + '</h4>'
			+ '</div>'
		...
		myimages_div_html += '        <div class="modal fade" id="myImage' + i + '" tabindex="-1" role="dialog" aria-labelledby="myModalLabe" aria-hidden="true">'
			...
			+ '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'
			+ '<h4 class="modal-title" id="myModalLable">图标修改--' + (i + 1) + '</h4>'
			+ '</div>'
		...
		http.getAjax_clean("/photo-album/manger/get_products_view", function (datanumber) {
			...
			for (var j = 0; j < data.length; j++) {
				...
				if (data[j].index != "") {
					...
					var actionaliasname_text;
					if (data[j].actionaliasname != null) {
						actionaliasname_text = data[j].actionaliasname
					} else {
						actionaliasname_text = '空';
					}
					promotion_imglist_html = ''
						+ '<input id="' + j + '" class="setup_input" style="width:205px;" value="' + data[j].aliasname + '" onkeydown="if(event.keyCode==13){changename(this);}" onBlur="changename(this);" ></input>'
						+ '<button class="myImage_button" data-toggle="modal" data-target="#myImage' + j + '">'
						+ '<img src="' + img_list + '" width="200px" height="200px"/>'
						+ '</button>'
						+ '<br>'
						+ '<div id="datanumber_display_' + j + '" align="center">' + actionaliasname_text + '</div>'
						+ '<button class="btn btn-primary" name="promo_set_btn"  data-toggle="modal" data-target="#myModal' + j + '">设置推广类别</button>';
...
function promotion_highspeed_submit(elemid) {
	var $checkeds = $('input[name="promotion_highspeed_check"]:checked');
