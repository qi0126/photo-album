  ...
  if(elem == null){
    ...
    if(elem.sizeInfo == null){
      ...
      $(elem.sizeInfo).each(function(k,kelem){
        ...
        var kweight = (kelem.weight == 'null' || kelem.weight == null)?'':kelem.weight;
        // console.log(klength);
        szTbodyHtml += '<tr>\
                            <td><input name="weight" type="text" style="width:90%;" value="'+kweight+'"/></td>\
  ...
  http.getAjax_clean("/photo-album/manger/get_products", function(data) {
    ...
    var post_data = new FormData();
      ...
      var proid_detail = data;
          ...
          // console.log(proid_detail.material);
          if(proid_detail.material != null){
            var material_html = '';
            $(proid_detail.material.split(',')).each(function(p,pelem){
              material_html += "<span class='tag'>" + pelem + "<button class='close' onclick='deltag(this)'>×</button></span>";
            })
            $('span[name=material]').html(material_html);
          }

          if(proid_detail.shapes != null){
            var shapes_html = '';
            $(proid_detail.shapes.split(',')).each(function(p,pelem){
              shapes_html += "<span class='tag'>" + pelem + "<button class='close' onclick='deltag(this)'>×</button></span>";
            })
            $('span[name=shapes]').html(shapes_html);
          }

          if(proid_detail.craft != null){
            var craft_html = '';
            $(proid_detail.craft.split(',')).each(function(p,pelem){
              craft_html += "<span class='tag'>" + pelem + "<button class='close' onclick='deltag(this)'>×</button></span>";
            })
            $('span[name=craft]').html(craft_html);
          }

          if(proid_detail.makeCraft != null){
            var makeCraft_html = '';
            $(proid_detail.makeCraft.split(',')).each(function(p,pelem){
              makeCraft_html += "<span class='tag'>" + pelem + "<button class='close' onclick='deltag(this)'>×</button></span>";
            })
            $('span[name=makeCraft]').html(makeCraft_html);
          }

          if(proid_detail.colour != null){
            var colour_html = '';
            $(proid_detail.colour.split(',')).each(function(p,pelem){
              colour_html += "<span class='tag'>" + pelem + "<button class='close' onclick='deltag(this)'>×</button></span>";
            })
            $('span[name=colour]').html(colour_html);
          }
          //推广类别获取
          $(proid_detail.generalize).each(function(l,lelem){
            $('#'+lelem).attr("checked",'true')
          });
          // console.log(data);
          $('#suppliernumber_div').html(proid_detail.date);
          $('#pro_id').html(proid_detail.id);
...
function newpro_btn_submit(){
  ...
  // console.log($('#img_list_null .img_listimg_div').length);

  // if(numer_input_text != '' && $('#img_list_null .img_listimg_div').length != 0){
    ...
    var imageInfo_list=imginfo_json;
...
function getKey(elem)
{
  ...
  //添加回车事件
  if(event.keyCode == 13){
   elem.value = tempvalue.substring(0, startPos) + '\r\n' + tempvalue.substring(endPos, tempvalue.length);
   cursorPos += 1;
   elem.selectionStart = elem.selectionEnd = cursorPos;
  }
  //添加空格事件
  if(event.keyCode == 32){
    elem.value = tempvalue.substring(0, startPos) + ' ' + tempvalue.substring(endPos, tempvalue.length);
    cursorPos += 1;
    elem.selectionStart = elem.selectionEnd = cursorPos;
  }
}
